...
export default function AdminNav({ salonName }: { salonName: string }) {
  ...
  return (
    <>
        ...
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden sm:block text-xs text-zinc-500">{today}</p>
            </div>
          </div>

          {/* Right: desktop nav links + Sign Out / mobile live clock */}
          <div className="flex items-center gap-2 shrink-0">
            ...
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="hidden sm:block rounded-full px-3 py-2 text-xs font-semibold text-zinc-600 transition hover:bg-zinc-100 hover:text-zinc-950"
              >
                {item.label}
              </Link>
            ))}
            ...
            <button
              ...
              className="hidden sm:flex min-h-9 items-center gap-1.5 rounded-xl bg-zinc-100 px-3 text-sm font-semibold text-zinc-700 transition-colors hover:bg-zinc-200 active:scale-95 shrink-0"
            >
            ...
            {/* Mobile: Live clock pill (replaces Sign Out) */}
            <div className="sm:hidden inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-zinc-100 text-xs font-medium text-zinc-700">
      ...
      {/* ── iOS Frosted Bottom Navigation Bar (mobile only) ─────────────── */}
      <div className="fixed bottom-0 left-0 right-0 z-40 sm:hidden backdrop-blur-xl bg-white/85 border-t border-black/[0.06] shadow-[0_-1px_3px_rgba(0,0,0,0.02)] pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-2 px-6 flex justify-around items-center">
        {NAV_ITEMS.map((item) => {
